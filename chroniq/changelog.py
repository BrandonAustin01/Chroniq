...
def ensure_changelog_exists() -> None:
    ...
    if not CHANGELOG_FILE.exists():
        try:
            ...
            print("📄 [cyan]CHANGELOG.md created successfully.[/cyan]")
        except Exception as e:
            print(f"❌ [red]Failed to create CHANGELOG.md:[/red] {e}")
...
def add_entry(version: str, message: str) -> None:
    ...
    if not message.strip():
        print("⚠️ [yellow]Skipped changelog update: message was empty.[/yellow]")
        return
    ...
    try:
        ...
        print("📝 [green]Changelog updated with version:[/green]", version)
    except Exception as e:
        print(f"❌ [red]Failed to write to changelog:[/red] {e}")
...
def get_recent_entries(limit: int = 5) -> List[str]:
    ...
    try:
        ...
        print(f"❌ [red]Error reading changelog:[/red] {e}")
        return []


# 🧪 Example usage (safe to test from CLI or shell):
